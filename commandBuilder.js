...
module.exports = {
  generateImageToVideoCommand({ imagePath, audio, shouldOverlayWhiteBackground, dimentions, scale, subtext, audioTrim, outputPath, silent, duration }) {
    ...
    if (scale) {
      command += `[outv];${generateScale('[outv]', scale)}`;
  ...
  generateVideoToVideoCommand({ videoPath, audio, audioDuration, videoDuration, subtext, outputPath, videoDimentions, frameRate, silent, duration, scale }) {
    ...
    if (silent) {
      ...
      if (scale) {
        command += `${generateScale('[0:v]', scale)}`;
        command += `[outv];${generateBackgroundBlur('[0:v]', '[bg]')};[bg][outv]overlay=(W-w)/2:(H-h)/2`;
      } else {
  ...
  generateGifToVideoCommand({ gifPath, audioDuration, audio, subtext, outputPath, silent, duration, scale }) {
    let command = '';
    if (silent) {
      command = `ffmpeg -y -ignore_loop 0 -t ${duration} -i ${gifPath} -f lavfi -i anullsrc=channel_layout=5.1:sample_rate=48000 -t ${duration} -c:v libvpx-vp9 -filter_complex "`;
      if (scale) {
        command += `${generateScale('[0:v]', scale)}`;
        command += `[outv];${generateBackgroundBlur('[0:v]', '[bg]')};[bg][outv]overlay=(W-w)/2:(H-h)/2`;
      } else {
...
function generateScale(inputStream, scale) {
  switch (scale) {
    case 'width':
      return `${inputStream}${constants.FFMPEG_SCALE_WIDTH}`;
    case 'height':
      return `${inputStream}${constants.FFMPEG_SCALE_HEIGHT}`;
    default:
    return `${inputStream}${constants.FFMPEG_SCALE_BOTH}`;
  }
}
