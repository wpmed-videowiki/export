...
module.exports = {
  generateImageToVideoCommand({ imagePath, audio, shouldOverlayWhiteBackground, dimentions, subtext, audioTrim, outputPath, silent, duration }) {
    ...
    if (silent) {
      command += ` -t ${duration} -f lavfi -i anullsrc=channel_layout=5.1:sample_rate=48000 -t ${duration} -pix_fmt yuv420p -filter_complex "`
    } else {
  ...
  generateVideoToVideoCommand({ videoPath, audio, audioDuration, videoDuration, subtext, outputPath, videoDimentions, frameRate, silent, duration }) {
    ...
    if (silent) {
      ...
      command = `ffmpeg -y -t ${audioDuration} -i ${videoPath} -i ${audio} -c:v libvpx-vp9 -c:a libvorbis -map 0:v:0 -map 1:a:0 -filter_complex "${constants.FFMPEG_SCALE}`;
      if (subtext) {
        command += `[outv];[outv]format=yuv444p[outv];[outv]drawbox=y=0:color=black@0.8:width=iw:height=30:t=max[outv];[outv]drawtext=text='${normalizeCommandText(subtext)}':fontcolor=white:fontsize=12:x=10:y=10[outv];[outv]format=yuv420p`
      }
  ...
  generateGifToVideoCommand({ gifPath, audioDuration, audio, subtext, outputPath, silent, duration }) {
    let command = '';
    if (silent) {
      command = `ffmpeg -y -ignore_loop 0 -t ${duration} -i ${gifPath} -f lavfi -i anullsrc=channel_layout=5.1:sample_rate=48000 -t ${duration} -c:v libvpx -crf 12 -b:v 500K -filter_complex "${constants.FFMPEG_SCALE}`;
    } else {
