...
// Keep stderr quiet: these commands run via exec, which kills the process
// if stdout/stderr exceeds maxBuffer. Real errors are still reported.
const QUIET_FLAGS = '-hide_banner -loglevel error -nostats';
...
module.exports = {
  generateImageToVideoCommand({ imagePath, audio, shouldOverlayWhiteBackground, dimentions, scale, subtext, audioTrim, outputPath, silent, duration }) {
    let command = `ffmpeg ${QUIET_FLAGS} -y -thread_queue_size 512 -framerate 25 -loop 1 -i "${imagePath}"`;
  ...
  generateVideoToVideoCommand({ videoPath, audio, audioDuration, videoDuration, subtext, outputPath, videoDimentions, frameRate, silent, duration, scale }) {
    let command;
    if (silent) {
      command = `ffmpeg ${QUIET_FLAGS} -y -t ${duration} -i "${videoPath}" -c:v libvpx-vp9 -pix_fmt yuv420p  -filter_complex "`;
      if (scale) {
      ...
      command = `ffmpeg ${QUIET_FLAGS} -y -t ${audioDuration} -i "${videoPath}" -i "${audio}" -c:v libvpx-vp9 -c:a libvorbis -map 0:v:0 -map 1:a:0 -filter_complex "${constants.FFMPEG_SCALE_BOTH}`;
      if (subtext) {
        command += `[outv];[outv]${generateSubtext(audioDuration, subtext)}[outv];[outv]format=yuv420p`
      }
      command += `" -shortest ${outputPath}`;
    } else {
      command = `ffmpeg ${QUIET_FLAGS} -y -f lavfi -i color=s=${videoDimentions}:d=${audioDuration}:r=${frameRate}:c=0xFFE4C4@0.0 -i "${videoPath}" -i "${audio}" -c:v libvpx-vp9 -c:a libvorbis -filter_complex "[0:v][1:v]overlay[outv];[outv]scale=w=800:h=600,setsar=${constants.SAR},setdar=${constants.DAR},pad=800:600:(ow-iw)/2:(oh-ih)/2[outv]`
      if (subtext) {
  ...
  generateGifToVideoCommand({ gifPath, audioDuration, audio, subtext, outputPath, silent, duration, scale }) {
    let command = '';
    if (silent) {
      command = `ffmpeg ${QUIET_FLAGS} -y -ignore_loop 0 -t ${duration} -i "${gifPath}" -t ${duration} -c:v libvpx-vp9 -filter_complex "`;
      if (scale) {
        command += `[0:v]${generateScale(scale)}`;
        command += `[outv];${generateBackgroundBlur('[0:v]')}[bg];[bg][outv]overlay=(W-w)/2:(H-h)/2`;
      } else {
      ...
      command = `ffmpeg ${QUIET_FLAGS} -y -ignore_loop 0 -t ${audioDuration} -i "${gifPath}" -i "${audio}" -filter_complex "${constants.FFMPEG_SCALE_BOTH}`;
    }
    if (subtext) {
      command += ` [outv];[outv]${generateSubtext(subtext)}[outv];[outv]format=yuv420p`
    }
